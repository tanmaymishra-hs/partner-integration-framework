...
import {createSubsAsync} from '../../../../utils/CreateSubsAsync'
...
export function Index() {
    ...
    const handleSubmit = async (event, setisLoading)=>{
      ...
      let partner_global_token = createSubsAsync(values["RequestBody"], secretKey)
      ...
      try{
       ...
       const response = await fetch(`https://partner-orchestrator-qa.pp.hotstar-labs.com/v2/partner/subscription/create-async`, {
        method: 'POST',
        headers,
  ...
  return (
    <FormElement obj = {config["config"]["apis"]["createSubscriptionAsync"]} title = "Create Subscription Async" description = "This Api helps you create Subscription asynchronously." handleSubmit={handleSubmit} values={values} setValues={setValues} result={result}/>
  )
}
